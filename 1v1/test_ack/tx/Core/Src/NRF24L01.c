...
/**
  ...
  *           
  */ 
uint8_t NRF24L01_TxPacket(uint8_t *txbuf,uint8_t *rxbuf)
{
	uint8_t sta;
	NRF24L01_CE_LOW();
  NRF24L01_Write_Buf(WR_TX_PLOAD,txbuf,TX_PLOAD_WIDTH);//写数据到TX BUF  32个字节
 	//NRF24L01_Write_Buf(NRF_WRITE_REG | TX_ADDR,(uint8_t*)TX_ADDRESS,RX_ADR_WIDTH);    //写自己的地址用于自动打包入发射帧，并使能ACK
  //NRF24L01_Write_Buf(NRF_WRITE_REG + RX_ADDR_P0,(uint8_t*)TX_ADDRESS,TX_ADR_WIDTH); //启用应答时通道0用于接收ACK  
  NRF24L01_CE_HIGH();                                  //启动发送	 
    ...
    printf("NRF24L01 MAX_TX\r\n"); 
		return MAX_TX; 
	}
	if(sta&TX_OK)                                        //发送完成
	{
    /****************************************************************/
    uint8_t rxlen = NRF24L01_Read_Reg(NRF_READ_REG + R_RX_PL_WID);
    if(rxlen < 33)
    {
      NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf,rxlen);
    }
    else
    {
      rxlen = 0;
    ...
    printf("NRF24L01 RX_OK:%d\r\n",rxbuf[0]); 
    /****************************************************************/
    printf("NRF24L01 TX_OK\r\n");
		return TX_OK;
	}
  if(sta&RX_OK)
  {
    printf("NRF24L01 RX_OK\r\n"); 
    uint8_t rxlen = NRF24L01_Read_Reg(NRF_READ_REG + R_RX_PL_WID);
    if(rxlen < 33)
    {
      NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf,rxlen);
  ...
  printf("NRF24L01 Fail because other reason\r\n"); 
 ...
 * 
 */
void NRF24L01_PacketAckData(uint8_t* pBuf,uint8_t len)
{
	NRF24L01_CE_LOW();	
